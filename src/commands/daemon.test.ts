...
describe('daemonStatus extension label states (#1575)', () => {
  ...
  function extensionLineFor(extra: Record<string, unknown>): string | undefined {
    fetchDaemonStatusMock.mockResolvedValue({
      ok: true,
      pid: 99,
      uptime: 60,
  ...
  it('uses singular grammar with one profile registered but no default', async () => {
    const line = await extensionLineFor({
      extensionConnected: false,
      profileRequired: true,
