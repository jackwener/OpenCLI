...
function normalizeSortBy(value) {
    ...
    if (!normalized)
        return 'top-sale';
    if (normalized === 'top-sale' || normalized === 'latest' || normalized === 'relevance')
        return normalized;
...
function buildSearchUrl(query, sortby, origin = DEFAULT_SHOPEE_ORIGIN) {
...
function canonicalizeProductUrl(href, baseUrl) {
...
function normalizeSearchRows(payload, limit) {
    ...
    for (const item of payload.items ?? []) {
        ...
        if (!productUrl || rowsByUrl.has(productUrl))
            continue;
        ...
        if (rowsByUrl.size >= limit)
            break;
    }
    return [...rowsByUrl.values()];
}
async function readSearchResults(page, query, sortby, limit, origin) {
    ...
    const payload = await page.evaluate(buildSearchExtractScript(Math.max(limit * 2, 20)));
    if (payload?.loginRequired) {
        throw new AuthRequiredError(new URL(origin).hostname, 'Shopee login required');
    }
    const rows = normalizeSearchRows(payload && typeof payload === 'object' ? payload : {}, limit);
    if (!rows.length) {
        throw new EmptyResultError('shopee search', 'No Shopee product links were found on the search page. Check login state, query, and page layout.');
    }
    return rows;
}
cli({
    ...
    func: async (page, args) => {
        ...
        if (!query)
            throw new Error('Shopee search query is required.');
