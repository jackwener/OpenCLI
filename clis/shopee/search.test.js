...
describe('shopee search URL and sort helpers', () => {
    ...
    it('builds a Shopee search URL for any Shopee origin', () => {
        expect(__test__.buildSearchUrl('camera', 'top-sale', 'https://shopee.com.my')).toBe('https://shopee.com.my/search?keyword=camera&page=0&sortBy=sales');
        expect(__test__.buildSearchUrl('kids camera', 'latest', 'shopee.sg')).toBe('https://shopee.sg/search?keyword=kids+camera&page=0&sortBy=ctime');
    });
...
describe('shopee product URL normalization', () => {
    ...
    it('canonicalizes slug product URLs and strips tracking params', () => {
        expect(__test__.canonicalizeProductUrl('/Kids-Mini-Camera-i.1385679855.27077262756?extraParams=1&sp_atk=abc#hash', sourceUrl)).toBe('https://shopee.com.my/Kids-Mini-Camera-i.1385679855.27077262756');
    });
    it('canonicalizes product path URLs', () => {
        expect(__test__.canonicalizeProductUrl('/product/1385679855/27077262756?foo=bar', sourceUrl)).toBe('https://shopee.com.my/product/1385679855/27077262756');
    });
    it('rejects non-product and non-Shopee URLs', () => {
        expect(__test__.canonicalizeProductUrl('/find_similar_products?catid=100635&itemid=27077262756&shopid=1385679855', sourceUrl)).toBe('');
...
describe('shopee search command execution', () => {
    it('navigates, extracts, and returns product links', async () => {
        ...
        expect(page.goto).toHaveBeenCalledWith('https://shopee.com.my/search?keyword=camera&page=0&sortBy=sales', { waitUntil: 'load' });
