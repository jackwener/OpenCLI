...
const { EXPORT_DIALOG_SELECTOR, EXPORT_REVIEW_BUTTON_SELECTOR, DETAIL_FILTER_INPUT_SELECTOR, TIME_PERIOD_START_INPUT_SELECTOR, TIME_PERIOD_START_MONTH_OFFSET, TIME_PERIOD_START_DAY_OFFSET, CONFIRM_EXPORT_BUTTON_SELECTOR, normalizeShopeeReviewUrl, bindShopeeProductTab, ensureShopeeProductPage, buildEnsureCheckboxStateScript, buildResolveTargetSelectorScript, buildReadInputValueScript, buildDispatchEnterOnInputScript, computeShiftedDateFromInputValue, buildWaitForExportReviewReadyScript, setComputedTimePeriodStartValue, } = await import('./product-shopdora-download.js').then((m) => m.__test__);
describe('shopee product-shopdora-download adapter', () => {
    ...
    it('computes the date from the input value using -3 months + 7 days', () => {
        ...
        expect(() => computeShiftedDateFromInputValue('not-a-date')).toThrow('Shopee product-shopdora-download could not parse the time-period start date');
    });
    it('clicks, computes from the current input value, types the result, and presses Enter', async () => {
        ...
        const page = { click, typeText, pressKey, nativeKeyPress, wait, evaluate };
        await expect(setComputedTimePeriodStartValue(page)).resolves.toBe('2026-01-21');
    ...
    it('binds to the matching existing browser tab using the shopee workspace', async () => {
        ...
        await expect(bindShopeeProductTab('https://shopee.sg/Jeep-EW121-True-Wireless-Bluetooth-5.4-Earbuds-i.1058254930.25483790400', bindFn)).resolves.toBe(true);
    ...
    it('reuses the matched tab, clears localStorage, and reloads the product page', async () => {
        ...
        const bindFn = vi.fn(async () => ({ tabId: 2 }));
        await expect(ensureShopeeProductPage(page, 'https://shopee.sg/product-i.1.2', bindFn)).resolves.toBe(true);
    ...
    it('navigates, downloads the file, and returns the local file url', async () => {
        ...
        const page = { goto, wait, click, typeText, pressKey, scroll, evaluate, waitForDownload };
        const result = await command.func(page, {
            url: 'https://shopee.sg/Jeep-EW121-True-Wireless-Bluetooth-5.4-Earbuds-i.1058254930.25483790400',
        });
        expect(goto).toHaveBeenCalledTimes(1);
        expect(goto).toHaveBeenNthCalledWith(1, 'https://shopee.sg/Jeep-EW121-True-Wireless-Bluetooth-5.4-Earbuds-i.1058254930.25483790400', { waitUntil: 'load' });
        ...
        const timedWaits = wait.mock.calls
            .map(([arg]) => (arg && typeof arg === 'object' ? Number(arg.time ?? 0) : 0))
    ...
    it('falls back to clearing the target host and reopening the product page when no existing product tab is found', async () => {
        ...
        await expect(ensureShopeeProductPage(page, 'https://shopee.sg/product-i.1.2', bindFn)).resolves.toBe(false);
        expect(page.goto).toHaveBeenCalledTimes(1);
        expect(page.goto).toHaveBeenNthCalledWith(1, 'https://shopee.sg/product-i.1.2', { waitUntil: 'load' });
    });
});
