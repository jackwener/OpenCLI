...
describe('xiaohongshu comments', () => {
    ...
    it('extracts shortform like counts from the shared xiaohongshu/rednote DOM script', async () => {
        ...
        expect(data.results).toEqual([
            { author: 'Alice', authorHref: '', text: 'Great note', likes: 21000, time: 'today', is_reply: false, reply_to: '' },
            { author: 'Bob', authorHref: '', text: 'Malformed count', likes: 0, time: '', is_reply: false, reply_to: '' },
        ]);
    });
    it('extracts authorHref from /user/profile/ anchor wrapping the name', async () => {
        const data = await runCommentsExtract(`
          <main>
            <section class="parent-comment">
              <div class="comment-item">
        ...
        expect(data.results[0].authorHref).toBe('/user/profile/5e8a1b2c3d4e5f6a7b8c9d0e?xsec_token=tok');
        expect(data.results[1].author).toBe('Bob');
        expect(data.results[1].authorHref).toBe('https://www.xiaohongshu.com/user/profile/abc123def456');
    });
    ...
    it('enriches each row with userId and profileUrl derived from authorHref', async () => {
        const page = createPageMock({
            loginWall: false,
            results: [
                { author: 'Alice', authorHref: '/user/profile/abc123?xsec_token=tok', text: 'hi', likes: 1, time: 't', is_reply: false, reply_to: '' },
                { author: 'Bob', authorHref: 'https://www.xiaohongshu.com/user/profile/xyz789', text: 'hey', likes: 0, time: '', is_reply: false, reply_to: '' },
                { author: 'Anon', authorHref: '', text: 'no link', likes: 0, time: '', is_reply: false, reply_to: '' },
            ],
        });
        const result = (await command.func(page, {
            'note-id': 'https://www.xiaohongshu.com/search_result/abc123?xsec_token=tok',
        ...
        // authorHref must not leak into the final row shape — it's an internal transport field
