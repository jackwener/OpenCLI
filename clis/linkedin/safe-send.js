...
async function probeThread(page) {
  const result = await page.evaluate(buildThreadProbeScript());
...
cli({
  ...
  func: async (page, args) => {
    ...
    const focus = await page.evaluate(buildFocusComposerScript());
    ...
    const composer = await page.evaluate(buildReadComposerScript());
    ...
    const sent = await page.evaluate(buildClickSendScript());
