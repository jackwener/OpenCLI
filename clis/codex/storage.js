// Browser-side state on the Codex App renderer (CDP 9238).
//
//   storage-keys [--storage local|session] [--filter]
//   storage-get <key> [--storage] [--max-bytes]
//   cookies      — list cookies for the codex renderer origin (app://-)
...
cli({
    ...
    name: 'storage-keys',
    access: 'read',
    description: 'List localStorage (default) or sessionStorage keys on the Codex App renderer, with byte sizes.',
    domain: 'localhost',
    ...
    func: async (page, kwargs) => {
      ...
      return out;
        ...
        if (!filtered.length) {
            throw new EmptyResultError('codex storage-keys', flt ? `No keys match "${flt}".` : `${store} is empty.`);
        }
...
cli({
    ...
    name: 'storage-get',
    access: 'read',
    description: 'Read a single localStorage / sessionStorage value on the Codex App renderer. Auto-decodes JSON.',
    domain: 'localhost',
    strategy: Strategy.UI,
    browser: true,
    args: [
...
cli({
    ...
    name: 'cookies',
    access: 'read',
    description: 'List cookies on the Codex App renderer (JS-visible via document.cookie; httpOnly cookies are not shown).',
    domain: 'localhost',
    strategy: Strategy.UI,
    browser: true,
    args: [],
    ...
    func: async (page) => {
        ...
        if (!raw) {
            throw new EmptyResultError('codex cookies', 'document.cookie is empty (Codex uses Electron session cookies, mostly httpOnly).');
        }
...
cli({
    ...
    name: 'idb-list',
    access: 'read',
    description: 'List IndexedDB databases on the Codex App renderer (app:// origin).',
    domain: 'localhost',
    strategy: Strategy.UI,
    browser: true,
    args: [],
    ...
    func: async (page) => {
      ...
      return await indexedDB.databases();
        ...
        if (!Array.isArray(dbs) || !dbs.length) {
            throw new EmptyResultError('codex idb-list', 'No IndexedDB databases (or browser does not support indexedDB.databases()).');
        }
        return dbs.map((d, i) => ({
            Index: i + 1,
