...
cli({
  ...
  access: "write",
  description:
    "Start a new Kimi chat (navigates to / with chat_enter_method=new_chat).",
  domain: KIMI_DOMAIN,
  strategy: Strategy.COOKIE,
  browser: true,
  siteSession: "persistent",
...
cli({
  ...
  access: "read",
  description:
    "List recent Kimi chats from the sidebar (with chat IDs extracted from href).",
  domain: KIMI_DOMAIN,
  strategy: Strategy.COOKIE,
  browser: true,
  siteSession: "persistent",
  navigateBefore: false,
  args: [{ name: "limit", type: "int", required: false, default: 30 }],
  columns: CHAT_COLUMNS,
  func: async (page, kwargs) => {
    ...
    const limit =
      Number.isInteger(kwargs?.limit) && kwargs.limit > 0 ? kwargs.limit : 30;
    return items
      .slice(0, limit)
      .map((r, i) => ({ Index: i + 1, Title: r.title, ChatId: r.id }));
  },
});
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    const limit =
      Number.isInteger(kwargs?.limit) && kwargs.limit > 0 ? kwargs.limit : 20;
    return turns
      .slice(0, limit)
      .map((t, i) => ({
        Index: i + 1,
...
cli({
  ...
  access: "read",
  description:
    "Read messages in the current Kimi chat. Pass --conv <id> to navigate to a specific chat first.",
  domain: KIMI_DOMAIN,
  strategy: Strategy.COOKIE,
  browser: true,
  siteSession: "persistent",
...
async function readKimiTurns(page) {
  return await page.evaluate(`(() => {
    ...
    for (const row of rows) {
      // Kimi nests toolcall containers inside an assistant message — the
      // thinking block (.toolcall-container.thinking-container) and any
      // web-search / tool-call results. They're the model's intermediate
      // reasoning, not the user-facing reply. The actual answer lives in
      // the .markdown-container that's NOT inside a .toolcall-container.
      // Strip the toolcall containers before extracting text so the captured
      // (and the 300-char preview) shows the answer, not the thinking trace.
      const clone = row.cloneNode(true);
      clone.querySelectorAll('.toolcall-container').forEach((el) => el.remove());
      const tx = (clone.textContent || clone.innerText || '').trim().replace(/\\s+/g, ' ');
...
async function sendKimiMessage(page, text) {
  ...
  if (!typeRes?.ok)
    throw new CommandExecutionError(
      typeRes?.reason || "composer type failed",
      "",
    );
  await page.wait(0.3);
  ...
  if (!sendRes?.ok)
    throw new CommandExecutionError(
      sendRes?.reason || "Send button click failed",
      "",
    );
...
cli({
  ...
  access: "write",
  description:
    "Send a message in the current Kimi chat (fire-and-forget; does not wait for reply).",
  domain: KIMI_DOMAIN,
  strategy: Strategy.COOKIE,
  browser: true,
  siteSession: "persistent",
...
cli({
  ...
  access: "write",
  description:
    "Return the text of the last assistant message. Pass --conv <id> to navigate to a specific chat first.",
  domain: KIMI_DOMAIN,
  strategy: Strategy.COOKIE,
  browser: true,
  siteSession: "persistent",
  ...
  func: async (page, kwargs) => {
    ...
    if (
      kwargs?.["click-button"] === true ||
      kwargs?.["click-button"] === "true"
    ) {
      const clickRes = await page.evaluate(clickBySvgNameScript("Copy"));
      if (!clickRes?.ok)
        throw new CommandExecutionError(
          clickRes?.reason || "Copy button not visible",
          "",
        );
    }
    return [
...
cli({
  ...
  access: "write",
  description:
    "Click Refresh (Kimi's regenerate button) on the last assistant message. Pass --conv <id> to target a specific chat.",
  domain: KIMI_DOMAIN,
  strategy: Strategy.COOKIE,
  browser: true,
  siteSession: "persistent",
  navigateBefore: false,
  args: [{ name: "conv", required: false, help: "Chat id or URL" }],
  columns: CHAT_COLUMNS,
  func: async (page, kwargs) => {
    await maybeNavigateConv(page, kwargs?.conv);
    ...
    if (!res?.ok)
      throw new CommandExecutionError(
        res?.reason || "Refresh button not visible",
        "",
      );
...
cli({
  ...
  access: "write",
  description:
    "Like or dislike the last assistant message. Pass --conv <id> to target a specific chat.",
  domain: KIMI_DOMAIN,
  strategy: Strategy.COOKIE,
  browser: true,
  siteSession: "persistent",
  ...
  func: async (page, kwargs) => {
    ...
    if (!res?.ok)
      throw new CommandExecutionError(
        res?.reason || `${kind} button not visible`,
        "",
      );
...
cli({
  ...
  access: "write",
  description:
    "Click Share on the last assistant message (opens Kimi's share dialog). Pass --conv <id> to target a specific chat.",
  domain: KIMI_DOMAIN,
  strategy: Strategy.COOKIE,
  browser: true,
  siteSession: "persistent",
  ...
  func: async (page, kwargs) => {
    ...
    if (!res?.ok)
      throw new CommandExecutionError(
        res?.reason || "Share button not visible",
        "",
      );
    await page.wait(1);
...
cli({
  ...
  access: "write",
  description:
    "Send a message and wait up to --timeout seconds for the assistant reply (best-effort: polls for turn count to grow + stabilize).",
  domain: KIMI_DOMAIN,
  strategy: Strategy.COOKIE,
  browser: true,
  siteSession: "persistent",
  ...
  func: async (page, kwargs) => {
    ...
    const timeoutSec =
      Number.isInteger(kwargs?.timeout) && kwargs.timeout > 0
        ? kwargs.timeout
        : 120;
    await ensureOnKimi(page);
    const baselineAssistant = (await readKimiTurns(page)).filter(
      (t) => t.role === "Assistant",
    ).length;
