...
async function fetchChatGPTConversationPayload(page, conversationId) {
    ...
    const errors = [];
    try {
        const cookieAuth = await buildChatGPTBackendHeaders(page, { includeAuthorization: false });
        if (cookieAuth.ok) {
            ...
            if (response.ok) {
                ...
                const bearerAuth = await buildChatGPTBackendHeaders(page, { includeAuthorization: true });
                if (bearerAuth.ok) {
