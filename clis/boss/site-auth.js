...
function commandColumns(config) {
  return ['logged_in', 'site', ...(config.columns ?? ['id', 'username', 'name'])];
}

function normalizeQuickCheck(result) {
  if (typeof result === 'boolean') return { logged_in: result };
  if (result && typeof result === 'object' && !Array.isArray(result)) return { logged_in: !!result.logged_in, ...result };
...
export function registerSiteAuthCommands(config) {
  ...
  cli({
    ...
    siteSession: 'persistent',
    args: [{ name: 'timeout', type: 'int', default: DEFAULT_TIMEOUT_SECONDS, help: 'Maximum seconds to wait for the user to finish login' }],
    ...
    func: async (page, kwargs) => {
      ...
      while (Date.now() < deadline) {
        ...
        try {
          return { status: 'login_complete', ...await tryProbe(config, page, 'poll') };
        } catch (error) {
          if (!isAuthRequired(error)) throw error;
          lastAuthMessage = getErrorMessage(error);
        }
      }
      throw new TimeoutError(`${config.site} login`, timeoutSeconds,
