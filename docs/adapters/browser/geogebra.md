...
| Command | Description |
|---------|-------------|
| `opencli geogebra eval "<cmd1>;<cmd2>;..."` | Execute one or more GeoGebra commands in a fresh automation page or an explicit workspace |
...
- Each `opencli geogebra ...` command runs in its own fresh browser session unless you pass `--workspace`.
- Passing `--workspace bound:<name>` makes the command run against that already-bound user tab instead of opening a fresh page.
...
opencli geogebra triangle --workspace bound:geogebra --size 4
opencli geogebra list --workspace bound:geogebra
opencli geogebra info --workspace bound:geogebra --name OCLIA
```

If you need raw browser JavaScript, you can still drop down to:
...
- If the user wants the current visible tab, bind first and then prefer `opencli geogebra ... --workspace bound:geogebra`.
