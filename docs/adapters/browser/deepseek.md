...
# If DeepSeek offers model selection, choose Expert
opencli deepseek ask "prove that sqrt(2) is irrational" --model expert

# If DeepSeek offers model selection, choose Vision for an image
...
# Combine DeepThink, web search, and a new chat
opencli deepseek ask "what happened today?" --think --search --new
...
| `--model` | Explicitly select `instant`, `expert`, or `vision` when the DeepSeek UI offers model selection; omitted by default |
...
- Without `--model`, `ask` uses DeepSeek's current default and does not require a model selector, including with `--new`. Explicit `--model` requires DeepSeek to offer model selection.
- DeepThink and Search are synced on every invocation, so omitting either flag disables that feature
- Expert and Vision modes do not support `--search`; omit `--model` to use DeepSeek's default for web search
