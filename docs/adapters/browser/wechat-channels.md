...
- 视频上传后需等待服务端转码，`waitForUploadDone` 默认最多等待 3 分钟；大文件可用 `--timeout`
  放宽命令整体超时。
...
- 定时发布通过 WeUI 桌面端日期-时间选择器完成；选择器结构变动时，命令会保存调试截图到
  `/tmp/channels_schedule_debug.png` 并降级为非定时发布。
