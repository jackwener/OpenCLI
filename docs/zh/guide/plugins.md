...
opencli plugin install https://gitlab.example.com/team/repo.git#subplugin   # 安装 monorepo 中的指定子插件
```
...
- 对于通用 git URL，`#subplugin` 匹配仓库根目录 `opencli-plugin.json`
  里的插件 key，实际目录由对应 entry 的 `path` 声明。
