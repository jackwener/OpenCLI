...
| `browser cookies [--domain <d>] [--url <u>]` | List cookies for the active tab as a JSON envelope `{cookies, count}`. Returns `name/value/domain/path/secure/httpOnly/sameSite/expirationDate/...` — including HttpOnly cookies. Use this to reuse a logged-in browser session in an external HTTP client (e.g. piping the output into `curl --cookie` or building a Python `httpx.Cookies` jar) without depending on `playwright-cli cookie-list`. `--url` is a CDP-side filter (only cookies that would be sent on requests to that URL); `--domain` is a client-side filter that matches the cookie's domain or any parent domain. |
