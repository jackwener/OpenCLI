...
| `browser cookies [--domain <d>] [--url <u>]` | List cookies as a JSON envelope `{cookies, count}`. With no filter, the command scopes to the active tab's current HTTP(S) URL; pass `--url` to scope to a specific request URL or `--domain` to export a domain jar. Returns `name/value/domain/path/secure/httpOnly/sameSite/expirationDate/...` — including HttpOnly cookies. Use this to reuse a logged-in browser session in an external HTTP client (e.g. piping the output into `curl --cookie` or building a Python `httpx.Cookies` jar) without depending on `playwright-cli cookie-list`. |
