...
| 站点 | 命令 |
|------|------|
| **xiaohongshu** | `search` `note` `comments` `notifications` `feed` `user` `download` `downloader` `publish` `creator-notes` `creator-note-detail` `creator-notes-summary` `creator-profile` `creator-stats` |
...
| 平台 | 内容类型 | 说明 |
|------|----------|------|
| **小红书** | 图片、视频、笔记元数据 | 下载笔记中的所有媒体文件；`downloader` 可调用本地 XHS-Downloader 服务 |
| **B站** | 视频 | 需要安装 `yt-dlp` |
